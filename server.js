...
if (!exists) {
  ...
  db.run('CREATE TABLE schedules (id INTEGER PRIMARY KEY AUTOINCREMENT, schedule TEXT, date VARCHAR(30), contact TEXT, media TEXT DEFAULT NULL, type TEXT DEFAULT NULL, namefile TEXT DEFAULT NULL, title TEXT, identity TEXT DEFAULT NULL, message TEXT, status BOOLEAN DEFAULT 0)');
}
      ...
      rows.forEach((schedule, i) => {
        if (!schedule.status) {
          cron.schedule(schedule.schedule, () => {
            ...
            stopFlag = false;
            emitInfoMessage();
            startLoop(message, title, identity, contacts, media);
          });
        }
      });
    }
...
app.post('/send', (req, res) => {
  ...
  statePromise.then((value) => {
    if (value === 'CONNECTED') {
      ...
      const query = `INSERT INTO schedules (schedule, date, contact, media, type, namefile, title, identity, message) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      const params = [
        cronFormat,
        req.body.schedule,
        req.body.upload0,
...
const sendProcess = async (i, messageBucket, titleMessage, identity, contacts, mediaFile) => {
...
async function startLoop(message, title, identity, contacts, media) {
  for (let i = 0; i < contacts.length; i++) {
    if (stopFlag) {
      break;
    }
    await delay(1200);
    sendProcess(i, message, title, identity, contacts, media);
  }
  let info = `
    <p class="w-1/2 text-center bg-emerald-500 hover:bg-emerald-600 rounded-lg mb-3 px-5 py-1 text-white text-xs">
